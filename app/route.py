...
def register_routes(app):
    ...
    def upload_image():
        try:
            ...
            if not request.is_json:
                print("錯誤：不是 JSON 格式")
                return jsonify({"error": "無效的內容類型，需要 JSON 格式"}), 415

            data = request.get_json()
            print(f"收到資料：{type(data)}")

            image_data = data.get("image")
            if not image_data or "," not in image_data:
                print("錯誤：圖片資料無效")
                return jsonify({"error": "圖片資料無效或遺失"}), 400
    ...
    def match_drug():
        try:
            ...
            print(f"比對參數 - 文字：{texts}, 顏色：{colors}, 形狀：{shape}")
            ...
            # 如果沒有文字或文字為 None
            if not texts or texts == ["None"] or not any(text.strip() for text in texts if text):
                print("無文字辨識，回傳所有候選藥品")
                results = []

                for _, row in df_sub.iterrows():  # 修復語法錯誤
                    # 檢查是否為無文字藥品
                    drug_text = str(row.get("文字", "")).strip()
                    if drug_text not in ["F:NONE|B:NONE", "F:None|B:None", "", "nan"]:
                        continue
            ...
            return jsonify({"error": "無法比對到對應的藥品"}), 404
